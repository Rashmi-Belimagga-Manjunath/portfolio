...
export default function OutsideSection() {
  return (
    <section className="py-20 sm:py-24 md:py-32 px-5 sm:px-8 md:px-10 max-w-[1100px] mx-auto">
      <FadeIn>
        <p className="text-[#E8573A] text-xs uppercase tracking-[0.2em] font-medium mb-3">Outside the Roadmap</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {items.map((item, i) => (
          <FadeIn key={item.title} delay={i * 0.1}>
            <div className="card rounded-2xl p-6 sm:p-8 h-full text-center">
              <span className="text-4xl mb-4 block">{item.emoji}</span>
              <h3 className="text-[#1A1A1A] font-bold text-base mb-2">{item.title}</h3>
